...
BivariateGUI <- function(){
  ...
  #add scrollbar
  scrx <- tk2scrollbar(tt, orient = "horizontal",
                       command = function(...) tkxview(topframe, ...))
  scry <- tk2scrollbar(tt, orient = "vertical",
                       command = function(...) tkyview(topframe, ...))
  #topframe<-tk2frame(tt,
   #             xscrollcommand = function(...) tkset(scrx, ...),
    #          yscrollcommand = function(...) tkset(scry, ...))
  topframe <- tk2text(tt, wrap = "none",
                xscrollcommand = function(...) tkset(scrx, ...),
               yscrollcommand = function(...) tkset(scry, ...))
  tkgrid(topframe, scry, sticky = "nsew")
  tkgrid.rowconfigure(tt, topframe, weight = 1)
  tkgrid.columnconfigure(tt, topframe, weight = 1)
  ...
  # Frame for Plotting
  plot.frame<-tk2frame(tt,borderwidth=2,relief="flat", padding=10)
